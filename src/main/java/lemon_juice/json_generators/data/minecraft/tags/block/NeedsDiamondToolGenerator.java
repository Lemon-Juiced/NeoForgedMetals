...
public class NeedsDiamondToolGenerator {
    public static void needsDiamondToolGenerator(){
        ArrayList<MetalResource> metalResources = MetalResourcesGenerator.generateMetalResources();
        File file = new File("src/main/resources/data/minecraft/tags/blocks/needs_diamond_tool.json");
        try {
            ...
            ArrayList<MetalResource> diamondLevelResources = new ArrayList<>();
            for (int i = 0; i < metalResources.size(); i++) {
                MetalResource currentIndex = metalResources.get(i);
                if (currentIndex.tier() == 3) diamondLevelResources.add(currentIndex);
            }

            for (int i = 0; i < diamondLevelResources.size(); i++) {
                MetalResource currentIndex = diamondLevelResources.get(i);
                ...
                if (i != diamondLevelResources.size() - 1) writer.println("    \"neoforged_metals:" + currentIndex.name() + "_block\",");
